...
def augment(scores, oracle_index):
    assert isinstance(scores, dy.Expression) # scores is of expression type
    shape = scores.dim()[0]
    assert len(shape) == 1
    increment = np.ones(shape)
    increment[oracle_index] = 0
    return scores + dy.inputVector(increment)
    # inputVector : Input a vector by values, returns corresponding _vecInputExpression (needed here 
    # because scores is of expression type)
